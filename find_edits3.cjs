    ...
    try {
        ...
        if (data.type === 'PLANNER_RESPONSE') {
            ...
            for (const tc of tool_calls) {
                if (tc.name === 'multi_replace_file_content' || tc.name === 'replace_file_content' || tc.name === 'default_api:multi_replace_file_content' || tc.name === 'default_api:replace_file_content') {
                    ...
                    if (args.TargetFile && args.TargetFile.includes('index.tsx')) {
                        ...
                        // Check if it's the hero or package changes we want
